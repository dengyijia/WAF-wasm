...
enum lookup_type { LOOKUP_WORD = 1, LOOKUP_TYPE = 2, LOOKUP_OPERATOR = 3, LOOKUP_FINGERPRINT = 4 };
...
typedef char (*ptr_lookup_fn)(struct libinjection_sqli_state*, int lookuptype, const char* word,
                              size_t len);
 ...
 *
 */
void libinjection_sqli_init(struct libinjection_sqli_state* sql_state, const char* s, size_t slen,
                            int flags);
 ...
 */
void libinjection_sqli_callback(struct libinjection_sqli_state* sql_state, ptr_lookup_fn fn,
                                void* userdata);

/*
 * Resets state, but keeps initial string and callbacks
 */
void libinjection_sqli_reset(struct libinjection_sqli_state* sql_state, int flags);
